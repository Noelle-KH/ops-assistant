...
export default function AdminSopPage() {
  ...
  const handleRenameCategory = async () => {
    if (!renameValue.trim() || !currentSop?.category) return;
    const oldName = currentSop.category;
    ...
    try {
      ...
      setAllCategories(prev => prev.map(c => 
        (c.name === oldName && c.type === "sop") ? { ...c, name: newName } : c
      ));
      ...
      setCurrentSop(prev => ({ ...prev!, category: newName }));
      setIsRenamingCategory(false);
    } catch (error) {
                    ...
                    <div className="space-y-3 col-span-1">
                      <Label className="text-[10px] font-black uppercase text-slate-500 tracking-widest flex justify-between">
                        ...
                        <div className="flex gap-2">
                          {!isAddingNewCategory && !isRenamingCategory && currentSop?.category && (
                            <button 
                              onClick={() => {
                                setIsRenamingCategory(true);
                                setRenameValue(currentSop.category || "");
                              }} 
