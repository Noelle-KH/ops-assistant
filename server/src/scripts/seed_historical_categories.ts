...
async function seedHistoricalCategories() {
  console.log("Seeding historical categories...");
  ...
  try {
    await db.insert(categories).values(historicalCategories);
    console.log("Historical categories seeded successfully!");
  } catch (e) {
    console.error("Failed to seed categories:", e);
  }
}

seedHistoricalCategories().catch(console.error);
